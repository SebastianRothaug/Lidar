...
This repository contains scripts for processing LiDAR point cloud data, mainly focusing on applications involving vegetation analysis regarding height, structure, and coverage.
...
* **01_powerline_modeling.py**
  * Modeling a powerline mathematically based on a symmetrical parabola calculation (2x pole location xyz + the zero point height; provided in an initial CSV).
    This results in an output of a powerline height model representing the electricity cable.
* **02_corridor_tree_detection.R**
  ...
  * Tree detection based on identifying the maxima of the Canopy Height Model - using a moving window approach with a dynamic window size for heterogeneous forest density and structure.
    This results in an output of a tree layer involving tree height information within the corridor.
...
* **Initial (external) data**, to initiate the scripts - marked with: **i** + the script number it needs to be implemented in.
  ...
  * **i02_lidar_full_aoi.las** - LiDAR point cloud of the area of interest.  **!! Due to data size issues, this specific example dataset couldn't be uploaded to the repository (contact me or alternatively continue with the already clipped AOI file: 02_lidar_aoi_clipped.las). !!**
...
<u>**My example data and project:**</u>
